...
function Mock() {
  ...
  // True when the PDF was auto-downloaded because the timer ran out
  // (vs. the user manually clicking "Finish Test & Download PDF")
  const [autoSubmittedByTimer, setAutoSubmittedByTimer] = useState(false);
  // Guards against firing the auto-download more than once
  ...
  // ─── Timer: when it hits zero, auto-download the PDF and open the feedback modal ───
  useEffect(() => {
    if (timeLeft <= 0) {
      setTimeLeft(0);
  ...
  const handleDownloadPDF = () => {
    ...
    doc.setFontSize(16);
    doc.text("Mock Exam Report", margin, 30);
    doc.setFontSize(8);
    ...
    doc.save(`WriteReady_Mock.pdf`);

    // Show feedback modal instead of immediate navigation
